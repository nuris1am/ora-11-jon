...
export function computeDashboardSummary(members = [], payments = [], investments = [], profitLoss = []) {
  ...
  // Total Lifetime Expected Target (Organization founded June 1, 2021)
  const startYear = 2021;
  const startMonth = 6; // June
  const totalMonthsInHistory = Math.max(1, (currentYear - startYear) * 12 + (currentMonth - startMonth + 1));
